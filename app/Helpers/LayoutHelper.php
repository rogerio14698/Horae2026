...
class LayoutHelper
{
    ...
    public static function makeBodyClasses()
    {
        ...
        // Fix for AdminLTE 3.14.3 bug - avoid array_push with empty arrays
        $layoutClasses = self::makeLayoutClasses();
        if (!empty($layoutClasses)) {
            array_push($classes, ...$layoutClasses);
        }
        
        $sidebarClasses = self::makeSidebarClasses();
        if (!empty($sidebarClasses)) {
            array_push($classes, ...$sidebarClasses);
        }
        
        $rightSidebarClasses = self::makeRightSidebarClasses();
        if (!empty($rightSidebarClasses)) {
            array_push($classes, ...$rightSidebarClasses);
        }
        
        $customBodyClasses = self::makeCustomBodyClasses();
        if (!empty($customBodyClasses)) {
            array_push($classes, ...$customBodyClasses);
        }
        
        $darkModeClasses = self::makeDarkModeClasses();
        if (!empty($darkModeClasses)) {
            array_push($classes, ...$darkModeClasses);
        }
    ...
    private static function makeLayoutClasses()
    {
        ...
        if (! self::isLayoutBoxedEnabled()) {
            $navbarClasses = self::makeFixedResponsiveClasses('navbar');
            $footerClasses = self::makeFixedResponsiveClasses('footer');
            
            // Fix for empty array bug - only push if arrays have content
            if (!empty($navbarClasses)) {
                array_push($classes, ...$navbarClasses);
            }
            if (!empty($footerClasses)) {
                array_push($classes, ...$footerClasses);
            }
        }
